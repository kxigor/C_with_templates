...
protected:
    void AssertToken(const Token& token, TokenType expected_type, StringView expected_lexem, uint32_t line, uint32_t col) {
        EXPECT_EQ(token.type, expected_type) << "Mismatched type for lexem: " << expected_lexem;
...
TEST_F(LexerTest, CompositeOperators) {
    ...
    AssertToken(tokens[3], TokenType::Error,    "!",  1, 9);
}
